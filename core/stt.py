...
class WhisperSTT:
    ...
    def __init__(self, model_name: str = "base", language: str | None = None):
        ...
        diagnostic(f"[STT] Loading Whisper '{model_name}'…")
        try:
            import torch
        ...
        try:
            ...
            if any(k in _e for k in _offline_keywords):
                diagnostic(f"[STT] Whisper '{model_name}' not in local cache — downloading (one-time, internet required)…")
        ...
        diagnostic(f"[STT] Whisper '{model_name}' ready ({device})")
    ...
    def transcribe(self, audio: np.ndarray) -> str:
        ...
        try:
            ...
            diagnostic(f"[STT] Transcription error: {e}")
            raise
...
class VoskSTT:
    ...
    def __init__(self, model_path: str | None = None, language: str = "en-us"):
        ...
        diagnostic("[STT] Loading Vosk model…")
        if model_path:
